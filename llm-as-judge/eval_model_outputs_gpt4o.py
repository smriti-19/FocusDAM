...
if __name__ == '__main__':
    # Example:
    # python eval_model_outputs_gpt4o.py --pred model_outputs_cache/cleaned_baseline_captions.json

    parser = argparse.ArgumentParser(description='Evaluate model outputs using OpenAI API (GPT models)')
    ...
    parser.add_argument('--api-call-limit', type=int, default=2000, help='API call limit')
    parser.add_argument('--api-key', type=str, default=None, help='Path to the OpenAI API key file (overrides .env.local)')
    parser.add_argument('--suffix', type=str, default="", help='Additional suffix for the evaluation file (appended after model name)')
    parser.add_argument('--model', type=str, default="gpt-4o-mini",
                        help='OpenAI model name (e.g., gpt-4o-mini, gpt-4o, gpt-4-turbo, gpt-3.5-turbo)')
    ...
    parser.add_argument('--base-url', type=str, default=None, help='Base URL for the API call (None for OpenAI default)')
    ...
    # Load API key from .env.local or command line argument
    api_key = os.getenv('OPENAI_API_KEY')
    if args.api_key:
        print(f"Loading API key from file: {args.api_key}")
    ...
    if args.base_url:
        ...
        print("Using OpenAI official API endpoint")
        client = OpenAI(api_key=api_key)

    # Generate output filename with model name
    # Format: <pred_base>_eval_<model_name><suffix>.json
    # Example: cleaned_baseline_captions_eval_gpt-4o-mini.json
    pred_base = os.path.splitext(args.pred)[0]
    model_name_safe = args.model.replace('/', '_').replace(':', '_')  # Sanitize model name for filename
    eval_file = f"{pred_base}_eval_{model_name_safe}{args.suffix}.json"
    eval_results = {}
    ...
    print(f"Total annotations to evaluate: {len(keys)}")
    print(f"API call limit: {args.api_call_limit}")
